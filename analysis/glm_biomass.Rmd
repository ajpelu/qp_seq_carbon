...
```{r}
df %>% 
  dplyr::select(code, wt_lid, ba,density, dem, slope, aspect, twi, sh_c2, n_strata) %>% 
...
plot_model(m, type = "pred", 
           ...
           ci.lvl = NA)
           +
             + theme_bw() + scale_y_continuous(lim=c(0,500))
...
pba + pdem + pden + psh + plot_layout(nrow = 2, ncol = 2)




visreg2d(m, "ba", "dem", scale = "response", plot.type = "persp", theta = -65, phi = -30)
...
summary(m1)
vif(m1)
# elimino n_strata --> muy correlacionado con sh_c2

m2 <- glm(wt_lid ~ ba + density + dem + slope + aspect + twi + sh_c2,
...
m3 <- glm.nb(wt_lid ~ ba + density + dem + slope + aspect + twi + sh_c2,
          data = df)
summary(m3)
...
df %>% 
  mutate(across(ba,density, dem, slope, aspect, twi, shc, n_strata), scale)

  pivot_longer(cols = ba:n_strata, names_to = "variables") %>% 
  ggplot(aes(x=wt_lid, y = value)) + 
  geom_point() + 
  facet_wrap(~variables)



mall <- glm(wt_lid ~ ba * density * dem * slope * aspect * twi * shc * n_strata, family = "gaussian", data = df)
summary(mall)
```

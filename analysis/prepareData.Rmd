...
# Data 
camarateData <- read_csv(here::here("raw_data/camarate/camarate_tree_data.csv"), col_types = cols()) %>% 
  filter(dis < 16) %>% 
   mutate(sp = case_when(
    sp == "Quercus pyrenaica" ~ "Qpyr",
...
camarateData %>% group_by(sp) %>% count()
...
plotsQP <- rbind(dendrolife_spa, dendrosn_spa,
           mig_spaFO, camarate, obsnev_sjer, robledal_lifespa,
           makeUniqueIDs = TRUE)  
```
...
mapview::mapview(qpR, color = "red", col.regions = "red", lwd = 1) + mapview::mapview(plotsQP, color = "blue", clo.regions = "blue")
```

# Generar datos 
```{r}
tree <-  bind_rows(
  list(mig_Data, dendrolife_Data, dendrosn_Data, camarateData, obsnev_sjerData, robledal_lifeData))
